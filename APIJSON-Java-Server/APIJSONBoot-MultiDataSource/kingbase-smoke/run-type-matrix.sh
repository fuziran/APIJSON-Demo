...
javac -encoding UTF-8 -d "$BUILD_DIR" "$SCRIPT_DIR/src/KingbaseTypeMatrixTest.java"

timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
report="$RESULT_DIR/type-matrix-$timestamp.log"
java -cp "$BUILD_DIR:$DRIVER_JAR" KingbaseTypeMatrixTest | tee "$report"
echo "Report: $report"
